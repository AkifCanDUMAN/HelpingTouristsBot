...
from Inline_Keyboard import Images2, LaraDudenWaterfall_URL,\
    HadrianCastleGate_URL
from Inline_Keyboard import Images3, IzmirClockTower_URL,\
    IzmirElevatorBuilding_URL
import logging
...
async def speak(message: Message):
    ...
    if message.text == '/Images':
        await message.answer('Which city would you like to go to',
                             reply_markup=Images)
    if message.text == '/Images2':
        await message.answer('Which city would you like to go to',
                             reply_markup=Images2)
    if message.text == '/Images3':
        await message.answer('Which city would you like to go to',
                             reply_markup=Images3)
...
async def get_AyasofyaCamii(call: CallbackQuery):
    ...
    await call.message.answer('You choise AyasofyaCamii',
                              reply_markup=AyasofyaCamii_URL)
...
async def get_SultanAhmetCamii(call: CallbackQuery):
    ...
    await call.message.answer('You choise SultanAhmetCamii',
                              reply_markup=SultanAhmetCamii_URL)
...
async def get_LaraDudenWaterfall(call: CallbackQuery):
    ...
    await call.message.answer('You choise LaraDudenWaterfall',
                              reply_markup=LaraDudenWaterfall_URL)
...
async def get_HadrianCastleGate(call: CallbackQuery):
    ...
    await call.message.answer('You choise HadrianCastleGate',
                              reply_markup=HadrianCastleGate_URL)
...
async def get_IzmirClockTower(call: CallbackQuery):
    ...
    await call.message.answer('You choise IzmirClockTower',
                              reply_markup=IzmirClockTower_URL)


@dp.callback_query_handler(text='IzmirElevatorBuilding')
async def get_IzmirElevatorBuilding(call: CallbackQuery):
    await call.answer(30)
    callback_data = call.data
    logging.info(f'call={callback_data}')

    await call.message.answer('You choise IzmirElevatorBuilding',
                              reply_markup=IzmirElevatorBuilding_URL)
